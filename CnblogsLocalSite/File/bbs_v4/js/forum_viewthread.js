/**
* weiphone bbs v4 forum_viewthread.js
* follow by discuz forum_viewthread.js
* kongge@office.feng.com
* 2013.09.12
*/


/** follow discuz forum_viewthread.js **/
var replyreload = '', attachimgST = [], zoomgroup = [], zoomgroupinit = [];
...
function appendreply(){
	var posElem = $('post_new');
	posElem.style.display = '';
	posElem.id = '';
	var posOffset = fetchOffset(posElem);
	if(window.ds && ds.scrollTo){
		ds.scrollTo(posOffset.top);
	}
	else{
		window.scrollTo(0, posOffset.top);
	}

	posElem = document.createElement('div');
	posElem.id = 'post_new';
	posElem.style.display = 'none';
	$('postlistreply').appendChild(posElem);
	if($('postform')) {
...
function toggleRatelogCollapse(tarId, ctrlObj){
	var target = jQuery('#' + tarId), isCollapse = !target.hasClass('rate_collapse');
	target[isCollapse ? 'addClass' : 'removeClass']('rate_collapse');
	ctrlObj.innerHTML = '<span>' + (isCollapse ? '展开' : '收起') + '<i class="collapse"></i></span>';
	setcookie('ratecollapse', isCollapse ? 1 : 0, isCollapse ? 2592000 : -2592000);
}
...
function display_blocked_post() {
	...
	$('hiddenposts').innerHTML+='<div id="postlistreply">'+postlistreply+'</div>';
}
...
/** end follow discuz forum_viewthread.js **/

/*
* 帖子页，显示用户名片，删除原 showauthor 函数
* kongge@office.feng.com
*/
jQuery(function($){
	var 
	prevTarget,
	inTimer, outTimer,
	minWidth = 136, maxWidth = 448,
	inDelay = 480, outDelay = 800,
	duration = 240,
	shell = $('#postlist'),
	triggerSeltor = 'div.avatar, .user_props>.tns';
	function hideCard(card){
		return card.stop().fadeOut(duration, function(){
			card.css('width', minWidth);
		});
	}
	shell.delegate(triggerSeltor, 'mouseenter', function(){
		clearTimeout(inTimer);
		
		var panel = $(this.parentNode);
		inTimer = setTimeout(function(){
			var 
			trigger = panel.find('div.avatar'),
			target = $('#' + trigger.attr('data-cardid'));
			
			if(!target.data('card_inited')){
				target.data('card_inited', true);
				var html = trigger.html();
				var tnsHTML = panel.find('.tns').html();
				if(tnsHTML){
					html += '<div class="tns">' + tnsHTML + '</div>';
				}
				$('#' + trigger.attr('data-cardid') + '_ma').html(html);
			...
			prevTarget = target.stop().css('opacity', 1).show().animate({width: maxWidth}, duration);
		}, inDelay);
	})
	.delegate(triggerSeltor, 'mouseleave', function(){
		clearTimeout(inTimer);
	});
});